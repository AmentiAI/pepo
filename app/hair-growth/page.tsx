import type { Metadata } from 'next';
import ProductCard from '@/components/ProductCard';
import { products } from '@/lib/products';

export const metadata: Metadata = {
  title: 'Buy Peptides for Hair Growth | GHK-Cu & TB-500',
  description: 'Buy peptides for hair growth — GHK-Cu, TB-500 and related follicle-support compounds.',
  alternates: { canonical: 'https://www.peptidesclav.com/hair-growth' },
  openGraph: {
    title: 'Buy Peptides for Hair Growth | GHK-Cu & TB-500',
    description: 'Buy peptides for hair growth — GHK-Cu, TB-500 and related follicle-support compounds.',
    type: 'website',
    url: 'https://www.peptidesclav.com/hair-growth',
    images: [{ url: '/og-image.png', width: 1200, height: 630, alt: 'Buy Peptides for Hair Growth' }],
  },
};

export default function HairGrowthPage() {
  const hairProducts = products.filter(
    (p) => p.tags?.includes('Hair Growth') || p.slug === 'ghk-cu' || p.slug === 'tb-500',
  );

  return (
    <div className="min-h-screen" style={{ backgroundColor: 'var(--bg)' }}>
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
        <h1 className="text-4xl sm:text-5xl font-extrabold text-gray-900 mb-10">
          Buy Peptides for Hair Growth
        </h1>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {hairProducts.map((product) => (
            <ProductCard key={product.slug} product={product} showAffiliateButton />
          ))}
        </div>
      </section>
    </div>
  );
}
