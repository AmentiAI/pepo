...
const categoryColors: Record<string, string> = {
  'Weight Loss': 'bg-violet-600/15 border-violet-600/25 text-violet-400',
  'Research': 'bg-cyan-500/15 border-cyan-500/25 text-cyan-400',
  'Protocols': 'bg-fuchsia-600/15 border-fuchsia-600/25 text-fuchsia-400',
  'Skinmaxxing': 'bg-pink-600/15 border-pink-600/25 text-pink-400',
  'Looksmaxxing': 'bg-amber-600/15 border-amber-600/25 text-amber-400',
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: 'var(--bg)' }}>
      ...
      <section className="py-10 sm:py-16 max-w-7xl mx-auto px-4 sm:px-6">
        <p className="text-xs font-semibold tracking-widest text-violet-400 uppercase mb-6">Latest Article</p>
          ...
          <div className="card p-8 border border-violet-600/20 relative overflow-hidden hover:border-violet-500/40 transition-colors">
            ...
            <div className="relative">
              <div className="flex items-center gap-3 mb-4">
                <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold border ${categoryColors[featured.category] ?? 'bg-zinc-800 border-zinc-700 text-zinc-400'}`}>
                  {featured.category}
                </span>
              ...
              <h2 className="text-2xl sm:text-3xl font-bold text-white mb-3 group-hover:text-violet-300 transition-colors leading-tight">
                {featured.title}
              </h2>
              <p className="text-zinc-400 mb-6 max-w-3xl leading-relaxed">{featured.description}</p>
      ...
      <section className="pb-10 sm:pb-20 max-w-7xl mx-auto px-4 sm:px-6">
        <p className="text-xs font-semibold tracking-widest text-zinc-500 uppercase mb-6">All Articles</p>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {rest.map((article) => (
            <Link key={article.slug} href={`/blog/${article.slug}`} className="group card p-6 flex flex-col hover:border-violet-500/30 transition-colors">
              <div className="flex items-center gap-2 mb-4">
                <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold border ${categoryColors[article.category] ?? 'bg-zinc-800 border-zinc-700 text-zinc-400'}`}>
                  {article.category}
                </span>
              ...
              <h3 className="font-bold text-white mb-2 leading-snug group-hover:text-violet-300 transition-colors flex-1">
                {article.title}
              </h3>
              <p className="text-sm text-zinc-500 mb-4 leading-relaxed line-clamp-2">{article.description}</p>
